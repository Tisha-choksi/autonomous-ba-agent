...
export function Sidebar() {
    ...
    return (
        <aside style={{
            ...
            background: "linear-gradient(180deg, #06101f 0%, #040c1a 100%)",
            ...
            <div style={{ padding: "20px 16px 16px", borderBottom: `1px solid ${C.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div>
                        <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 15.5, color: C.text, letterSpacing: "-0.01em" }}>
                            BA Agent
                        </div>
                ...
                {session ? (
                    <div style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                            ...
                            <span style={{ fontSize: 10, color: C.cyan, fontWeight: 500, letterSpacing: "0.05em" }}>ACTIVE DATASET</span>
                        </div>
                        <div style={{ fontSize: 12.5, color: C.text, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 3 }}>
                            {session.file_name}
                        </div>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: C.muted, display: "flex", gap: 8 }}>
                            <span style={{ display: "flex", alignItems: "center", gap: 3 }}>
                                <Database size={9} color={C.muted} />
                    ...
                    <div style={{
                        ...
                        <span style={{ fontSize: 11.5, color: C.muted }}>No dataset loaded</span>
                    </div>
                )}
            </div>
            ...
            <nav style={{ flex: 1, padding: "16px 10px", overflowY: "auto" }}>
                <div style={{ fontSize: 10, color: C.muted, letterSpacing: "0.09em", padding: "0 6px 8px", fontWeight: 500 }}>
                    NAVIGATION
                </div>
                        ...
                        <Link key={href} href={href} style={{ textDecoration: "none", display: "block", marginBottom: 2 }}>
                            <div className={`sidebar-link${active ? " active" : ""}`}>
                                <div style={{
                                    ...
                                    <Icon size={14} color={active ? C.cyan : C.muted} />
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontSize: 13.5, lineHeight: 1.2 }}>{label}</div>
                                    <div style={{ fontSize: 10.5, color: C.muted, marginTop: 1 }}>{desc}</div>
                                </div>
                                {active && <ChevronRight size={12} color={C.cyan} style={{ flexShrink: 0, opacity: 0.6 }} />}
                            </div>
                        </Link>
                    );
                })}
                ...
                <div style={{ padding: "0 6px" }}>
                    <div style={{ fontSize: 10, color: C.muted, letterSpacing: "0.09em", marginBottom: 8, fontWeight: 500 }}>
                        AI STATUS
                    </div>
                    {STATUS.map(({ label, status, color }) => (
                        <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 2px" }}>
                            ...
                            <span style={{ fontSize: 10, color, fontWeight: 600, letterSpacing: "0.02em" }}>{status}</span>
                        </div>
                    ))}
                </div>
            </nav>
            ...
            {/* ── Footer ── */}
            <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.border}`, background: "rgba(0,0,0,0.25)" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                    <Cpu size={10} color={C.muted} />
                    <span style={{ fontSize: 10.5, color: C.muted, letterSpacing: "0.03em" }}>
